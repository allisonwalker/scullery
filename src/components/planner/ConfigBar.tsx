...
export default function ConfigBar({
  ...
  // Shared pill renderer used in both desktop bar and mobile expanded panel
  function MealTypePill({ type }: { type: MealType }) {
    const isHidden = hiddenTypes.includes(type)
    if (isHidden) {
      return (
        <div className="flex items-center gap-1.5 bg-brand-900/30 rounded-lg px-2.5 py-1.5">
          ...
          <button onClick={() => toggleColumnVisibility(type)} title="Show column" className="ml-0.5 text-brand-500 hover:text-brand-200 transition-colors">
            <Eye size={11} />
          </button>
        </div>
      )
    ...
    return (
      <div className="group flex items-center gap-1.5 bg-brand-700/60 hover:bg-brand-700 rounded-lg px-2.5 py-1.5 transition-colors">
        <span className="text-brand-200">{MEAL_ICONS[type]}</span>
        <span className="text-xs text-brand-100">{MEAL_LABELS[type]}</span>
        <div className="flex items-center gap-0.5 ml-1">
          <button onClick={() => adjustCount(type, -1)} className="w-4 h-4 flex items-center justify-center text-brand-300 hover:text-white rounded transition-colors text-xs leading-none">−</button>
          <span className="w-4 text-center text-xs font-bold text-white">{config[type]}</span>
          <button onClick={() => adjustCount(type, 1)} className="w-4 h-4 flex items-center justify-center text-brand-300 hover:text-white rounded transition-colors text-xs leading-none">+</button>
        </div>
        <button onClick={() => toggleColumnVisibility(type)} title="Hide column" className="opacity-0 group-hover:opacity-100 ml-0.5 text-brand-400 hover:text-white transition-all">
          <EyeOff size={10} />
        </button>
      </div>
    )
  ...
  return (
    <div className="bg-brand-800 border-b border-brand-900">
      ...
      <div className="px-3 sm:px-4 py-2 flex items-center gap-1 sm:flex-wrap justify-between">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <div className="flex items-center gap-0.5 bg-brand-900/50 rounded-lg px-1.5 py-1 mr-1">
            <button onClick={() => navigate(-1)} className="w-6 h-6 flex items-center justify-center text-brand-300 hover:text-white rounded transition-colors" aria-label="Previous week">
              <ChevronLeft size={14} />
            </button>
            <span className="text-xs font-semibold text-white min-w-[120px] sm:min-w-[136px] text-center px-1 tabular-nums">
              {weekLabel(weekStart)}
            </span>
            <button onClick={() => navigate(1)} className="w-6 h-6 flex items-center justify-center text-brand-300 hover:text-white rounded transition-colors" aria-label="Next week">
              <ChevronRight size={14} />
            </button>
          </div>

          {/* Meal types + source mix + use-up — hidden on mobile, shown on sm+ */}
          <div className="hidden sm:flex items-center gap-1 flex-wrap">
            <div className="w-px h-5 bg-brand-600 mx-1.5" />

            {MEAL_TYPES.map((type) => <MealTypePill key={type} type={type} />)}
            ...
            <div className="relative">
              ...
              {showRatioSlider && (
                <div className="absolute top-full left-0 mt-2 z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-4 w-60">
                  ...
                  <input type="range" min={0} max={100} step={10} value={libraryPct} onChange={(e) => adjustRatio(Number(e.target.value))} className="w-full accent-brand-500" />
                  <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                    <span>All AI</span><span>All library</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative">
              ...
              >
                ...
                {useUpCount > 0 && (
                  <span className="text-[10px] bg-amber-400 text-amber-900 font-bold px-1.5 py-0 rounded-full leading-4">{useUpCount}</span>
                )}
              ...
              {showIngredients && (
                <div className="absolute top-full left-0 mt-2 z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-4 w-72">
                  ...
                  <p className="text-xs text-gray-400 mb-3">Claude will prioritise these when suggesting new recipes.</p>
                  {useUpCount > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {(config.ingredients_to_use ?? []).map((ing) => (
                        <span key={ing} className="flex items-center gap-1 text-xs bg-brand-50 text-brand-800 border border-brand-200 px-2 py-0.5 rounded-full">
                          {ing}
                          <button onClick={() => removeIngredient(ing)} className="text-brand-400 hover:text-brand-700"><X size={10} /></button>
                        </span>
                      ))}
                    </div>
                  )}
                  <div className="flex gap-2">
                    <input ref={ingredientInputRef} type="text" value={ingredientInput} onChange={(e) => setIngredientInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addIngredient()} placeholder="e.g. heavy cream" className="input text-sm flex-1" />
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className={cn(
              'sm:hidden flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 transition-colors',
              mobileExpanded
                ? 'bg-brand-600 text-white'
                : 'bg-brand-700/60 hover:bg-brand-700 text-brand-200',
            )}
          ...
          >
            ...
            <span className="sm:hidden">{isRegenerating ? '…' : 'Go'}</span>
          </button>
        </div>
      </div>
      ...
      {mobileExpanded && (
        <div className="sm:hidden border-t border-brand-900/60 px-3 pb-3 pt-2 space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder="e.g. quick Asian meals"
              className="bg-brand-700/60 border border-brand-600 rounded-lg px-2.5 py-1.5 text-xs text-white
                         placeholder:text-brand-400 focus:outline-none focus:border-brand-400 flex-1 min-w-0"
            />
          </div>
        </div>
      )}
